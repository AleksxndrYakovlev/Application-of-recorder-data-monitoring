...
bool ModbusConnection::modbus_read(Device^ dev) // Считывание по MODBUS TCP/IP
{
	uint16_t data[8001];
	...
	if (error != -1)
	{
		...
		if (data[start + 3] != 0)
		{
			for (int i = start; i < 7987; i += 4)  // Обработка полученных данных
			{
		...
		{
			for (int i = 1; i < start-3; i += 4)  // Обработка полученных данных
			{
				if (data[i + 3] != 0)
				{
					month = data[i] & mask_month;
